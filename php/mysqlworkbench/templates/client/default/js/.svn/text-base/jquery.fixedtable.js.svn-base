...
*/
var _fixedTableOldSource = new Array;
(function($) {
    // ***********************************************
    ...
    $.fn.fixedTable = function(opts) {
        ...
        _fixedTableOldSource[this.attr('id')] = $(mainid).html();
        var tbl = this;
        ...
        if (options.arrWidth) {
          if ( !($.browser.msie && parseInt($.browser.version) <= 8) ) {
            adjustResizable(options);
          }
          adjustSizesByArray(options); 
          adjustSizes2(options); 
        } else {
          adjustSizes(options); 
          if ( !($.browser.msie && parseInt($.browser.version) <= 8) ) {
            adjustResizable(options);
        ...
        $('#' + options.outerId).css('overflow', 'auto');
        $('#' + options.outerId).css('width', $('#' + options.outerId).width() + 'px');
        return tbl;
    }
    $.fn.fixedTableDestroy = function(outerClass) {
      this.parents(outerClass).html(_fixedTableOldSource[this.attr('id')]);
    }
        ...
        if ($(section, src).length) {
            ...
            $(section + " tr", src).each(function() {
                ...
                $(cellType + ":lt(" + cols + ")", this).each(function() {
                  if (cellType == "th") {
                    $("<td><div>" + $(this).html() + "<div></td>").addClass(this.className).attr("id", this.id).appendTo(tr);
                  } else {
        ...
        if ($(section, src).length) {
            ...
            $(section + " tr", src).each(function() {
                ...
                $(cellType, this).each(function() {
                  if (cellType == "th") {
                    $("<td><div>" + $(this).html() + "<div></td>").appendTo(tr);
                  } else {
    ...
    function adjustSizes(options) {
        ...
        if (contenttbH < maintbheight) {
            ...
            $("#" + Id + " .fixedContainer ." + options.classHeader).width($("#" + Id + " .fixedContainer ." + options.classHeader).width() + 17);
    ...
    function adjustSizesByArray(options) {
      var arrWidth = options.arrWidth;
      var totalWidth = 0;
      var Id = options.outerId;
      for (i = 0; i < options.fixedColumns; i++) {
        $('#' + Id + ' .' + options.classColumn + ' .' + options.classHeader + ' table tbody tr').find('td:eq(' + i + ')').css('width', arrWidth[i] + 'px');
        $('#' + Id + ' .' + options.classColumn + ' .fixedTable table tbody tr').find('td:eq(' + i + ')').css('width',arrWidth[i] + 'px');
        if ( !($.browser.msie && parseInt($.browser.version) <= 8) ) {
          $('#' + Id + ' .' + options.classColumn + ' .' + options.classHeader + ' table tbody tr').find('td:eq(' + i + ') div:first').css('width',arrWidth[i] + 'px');
          $('#' + Id + ' .' + options.classColumn + ' .' + options.classHeader + ' table tbody tr').find('td:eq(' + i + ') div:first').resizable({minWidth: arrWidth[i]});
        }
      }
      for (i = options.fixedColumns; i < arrWidth.length; i++) {
        j = i - options.fixedColumns;
        $('#' + Id + ' .fixedContainer .' + options.classHeader + ' table tbody tr').find('td:eq(' + j + ')').css('width', arrWidth[i] + 'px');
        $('#' + Id + ' .fixedContainer .fixedTable table tbody tr').find('td:eq(' + j + ')').css('width',arrWidth[i] + 'px');
        if ( !($.browser.msie && parseInt($.browser.version) <= 8) ) {
          $('#' + Id + ' .fixedContainer .' + options.classHeader + ' table tbody tr').find('td:eq(' + j + ') div:first').css('width',arrWidth[i] + 'px');
          $('#' + Id + ' .fixedContainer .' + options.classHeader + ' table tbody tr').find('td:eq(' + j + ') div:first').resizable({minWidth: arrWidth[i]});
        }
        var object = $('#' + Id + ' .fixedContainer .' + options.classHeader + ' table tbody tr:eq(0) td:eq(' + j + ')');
        var padding = (object.innerWidth() - object.width());
        totalWidth += arrWidth[i] + padding;
      }
      totalWidth += arrWidth.length + 1;
      $('#' + Id + ' .fixedContainer .' + options.classHeader + ' table').css('width', totalWidth + 'px');
      $('#' + Id + ' .fixedContainer .fixedTable table').css('width', totalWidth + 'px');
    }
    
    function adjustResizable(options) {
      var Id = options.outerId;
      $('#' + Id + ' .' + options.classColumn + ' .' + options.classHeader + ' table tbody td div').each(function(){
        var minWidth = $(this)[0].offsetWidth;
        $(this).resizable({
          alsoResize: '#' + Id + ' .' + options.classColumn + ', #' + Id + ' .' + options.classColumn + ' .fixedTable, #' + Id + ' .' + options.classColumn + ' .' + options.classHeader + '  table,#' + Id + '  .' + options.classColumn + ' .fixedTable  table',
          handles: "e",
          minWidth: $(this)[0].offsetWidth,
          resize: function (event, ui) {
            $('#' + Id + ' .' + options.classColumn + ' .' + options.classHeader + '  table tbody tr td').each(function(){
              $(this).width($('div',this)[0].offsetWidth + 'px');
              $('#' + Id + ' .' + options.classColumn + ' .' + options.classHeader + '  table').width( $('#' + Id + ' .' + options.classColumn + ' .' + options.classHeader + '')[0].offsetWidth + 'px');
              $('#' + Id + ' .' + options.classColumn + ' .fixedTable  table tbody tr:eq(0) td:eq('+$(this).index()+')').width( ($('div',this)[0].offsetWidth) + 'px');
              $('#' + Id + ' .' + options.classColumn + ' .fixedTable  table').width( $('#' + Id + ' .' + options.classColumn + ' .' + options.classHeader + '')[0].offsetWidth + 'px');
              $('#' + Id + ' .' + options.classColumn + ' .' + options.classFooter + '  table tbody tr:eq(0) td:eq('+$(this).index()+')').width( ($('div',this)[0].offsetWidth) + 'px');
              $('#' + Id + ' .' + options.classColumn + ' .' + options.classFooter + '  table').width( $('#' + Id + ' .' + options.classColumn + ' .' + options.classHeader + '')[0].offsetWidth + 'px');
            });
            var newWidth = $('#' + Id + ' .' + options.classColumn).width() + $('#' + Id + ' .fixedContainer').width();
            $('#' + Id + ' .fixedArea').width(newWidth);
          }
        });
      });
      $('#' + Id + ' .fixedContainer .' + options.classHeader + ' table tbody td div').each(function(){
        var minWidth = $(this)[0].offsetWidth;
        $(this).resizable({
          alsoResize: '#' + Id + ' .fixedContainer .' + options.classHeader + '  table,#' + Id + '  .fixedContainer .fixedTable  table',
          handles: "e",
          minWidth: minWidth,
          resize: function (event, ui) {
            var i = ui.element.parent('td').index();
            $('#' + Id + ' .fixedContainer .' + options.classHeader + '  table tbody tr td').each(function(){
                $(this).width($('div',this)[0].offsetWidth + 'px');
                $('#' + Id + ' .fixedContainer .fixedTable  table tbody tr:eq(0) td:eq('+$(this).index()+')').width( ($('div',this)[0].offsetWidth) + 'px');
                $('#' + Id + ' .fixedContainer .fixedTable  table').width( $('#' + Id + ' .fixedContainer .' + options.classHeader + ' table')[0].offsetWidth + 'px');
                $('#' + Id + ' .fixedContainer .' + options.classFooter + '  table tbody tr:eq(0) td:eq('+$(this).index()+')').width( ($('div',this)[0].offsetWidth) + 'px');
                $('#' + Id + ' .fixedContainer .' + options.classFooter + '  table').width( $('#' + Id + ' .fixedContainer .' + options.classHeader + ' table')[0].offsetWidth + 'px');
            });
          }
        });
      });
      $('#' + Id + ' .fixedContainer .' + options.classHeader + ' table tbody td div').each(function(){
        if ($.browser.msie && $.browser.version <= 8){
          $(this).width($('#' + Id + ' .fixedContainer .fixedTable  table tbody tr:eq(0) td:eq('+$(this).parent('td').index()+')').width());
        } else {
          $(this).width($(this)[0].offsetWidth + 1 + 'px');
          $('#' + Id + ' .fixedContainer .fixedTable  table tbody tr:eq(0) td:eq('+$(this).parent('td').index()+')').width($(this).width() + 'px');
        }
      });
      $('#' + Id + ' .' + options.classColumn + ' .' + options.classHeader + ' table tbody td div').each(function(){
        if ($.browser.msie && $.browser.version <= 8){
          $(this).width($('#' + Id + ' .' + options.classColumn + ' .fixedTable  table tbody tr:eq(0) td:eq('+$(this).parent('td').index()+')').width());
        } else {
          $('#' + Id + ' .' + options.classColumn + ' .fixedTable  table tbody tr:eq(0) td:eq('+$(this).parent('td').index()+')').width( ($(this)[0].offsetWidth) + 'px');
        }
      });
    }
})(jQuery);