...
(function(jQuery) {
	...
	jQuery.mydownloadfile = function(type,sql,file){
		$.post(
				'/phpexport/phppdf/download',
				{	type		:	JSON.stringify(type)
				,	sql			:	JSON.stringify(sql)
				,	file		:	JSON.stringify(file)
				},
			    function(res) {
					if (res=="blank"|| res=="" || jQuery.getExtension(res)!="pdf"){
						jAlert('No file to download', '', function(){
						});
					}else{
						window.location.replace('/phpexport/phppdf/getfile/url_file/'+encodeURIComponent(res));
					}
					return false;
			}
		);
 ...
 */
	jQuery.previewfilepdf = function(type,sql,file){
		$.colorbox({							// initialize colorbox plugin for showing assignee moving popup
    		iframe: true
        , 	href:'/phpexport/phppdf/preview/type/'+JSON.stringify(type)+'/sql/'+JSON.stringify(sql)+'/file/'+JSON.stringify(file)
		, 	innerWidth:1000
		, 	innerHeight: 540
		, 	opacity: 0.1
		, 	cbox_load: function(){
	 ...
	 * @example $.appendDatepicker($('#date-input'));
	 */
	jQuery.appendYmpicker = function(object) {
		try {
			...
			object.each(function(index, dom) {
				try {
					...
					dummy.ympicker({
						...
						onSelect : function(dateText, inst) {
							try {
								jQuery(this).trigger('change').blur()
										.focusout().focus();
								jQuery(this).datepicker('hide');
							} catch (e) {
								jQuery.showErrorDetail(e,
	 ...
	 *            text エラー内容
	 * @public
	 * @requires _phpFile['errorLog'] で定義された PHP ファイル
	 * @requires jQuery.getPath
	 * @example $.writeErrorLog(text);
	 */
	jQuery.writeErrorLog = function(text) {
		try {
			new jQuery.ajax({
				url : jQuery.getPath('php') + _phpFile['errorLog'],
				data : {
					'error' : text
				},
				type : 'post',
	 ...
	 */
	function _balloontipMouseover(event, object) {
		try {
			// 表示
			...
			if (balloontipMessage !== '') {
				...
				object.parents('body').append(
						'<p id="' + _balloontipId + '">' + balloontipMessage
								+ '</p>');
				jQuery('#' + _balloontipId, object.parents('body')).fadeIn(300,
						null);
			}
		} catch (e) {
	 ...
	 */
	function _balloontipMousemove(event, object) {
		try {
			var balloontip = jQuery('#' + _balloontipId, object.parents('body'));
			...
			jQuery('#' + _balloontipId, object.parents('body')).css(css);
		} catch (e) {
	 ...
	 */
	jQuery.fn.balloontip = function(message) {
		try {
